<script setup lang="ts">
    ...
    import { voucherClient, orderClient } from '@/constant'
    ...
    const onManualSubmit = handleSubmit(
        (values) => {
            ...
            change_detail_voucher(values)
            //
            toast.success('Đặt hàng thàn công')
            setTimeout(() => {
    ...
    const money_discount = computed(() => {
        ...
        if (voucher.value?.type === 'money') {
            return voucher.value?.money_discount
        }

        return 0
    })
    const total = computed(() => subtotal.value - money_discount.value + money_ship.value)
    const name = computed(() => {
    ...
    const change_detail_voucher = (data) => store.commit('client/order/CHANGE_DETAIL_ORDER', data)
    ...
    const startTime = ref("2025-08-26 20:40:35");
...
<template>
    <div class="bg-gray-300 dark:bg-gray-900 pb-10 h-[100vh]">
        <div class="grid grid-cols-12 max-w-7xl m-auto px-5 gap-x-15 pt-10">
            <div class="col-span-7  pb-10">
            ...
            <div class="col-span-5 bg-white dark:bg-gray-800 pb-10 px-15 rounded-sm">
        ...
        <div v-show="show_banking"  class="fixed top-0 right-0 bottom-0 left-0 "> 
            <div @click.prevent="show_banking = false" class="flex bg-gray-200 h-full justify-center items-center">
                <div @click.stop class="grid grid-cols-12 bg-white shadow-xl/30 rounded-sm w-6xl p-5 font-(family-name:--font-winky)">
                    ...
                    <div class="col-span-7 ">
                        ...
                        <button @click="show_info_banking =true" class="text-white bg-[var(--maincolor)] py-3 px-5 mt-2 rounded-ssm cursor-pointer ">Lấy thông tin thanh toán</button>
                        <div v-if="show_info_banking" class="border-t-1 mt-5 ">
                            ...
                            <div class="flex justify-between">
                                ...
                                <img class="w-60 h-60" src="/public/images/QRCUONG.jpeg" alt="">
                            </div>
                    ...
                    <div class="col-span-5">
                        ...
                        <div class="flex items-center mt-2">
                            <input type="text" class="flex-1 border-1 outline-0 rounded-sm p-3" placeholder="Mã giảm giá">
                            <button class="py-3 px-6 cursor-pointer bg-[var(--maincolor)] text-white rounded-sm ">Áp dụng</button>
                        </div>
                    </div>
                </div>
            </div>
