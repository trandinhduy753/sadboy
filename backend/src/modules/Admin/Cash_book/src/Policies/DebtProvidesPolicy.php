...
class DebtProvidesPolicy
{
    ...
    protected function hasPermission(Employee $employee, string $action)
    {
        ...
        $debtsPerms = $permissions['cash_book'];
        if (in_array('all', $debtsPerms)) {
            return true;
        }

        return in_array($action, $debtsPerms);
    }
    public function __construct()
    {
        //
    }
    public function viewDebt(Employee $employee)
    {
        return $this->hasPermission($employee, 'viewDebt');
    }
}
