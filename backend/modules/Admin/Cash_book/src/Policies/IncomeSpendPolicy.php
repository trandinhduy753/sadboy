...
class IncomeSpendPolicy
{
    ...
    protected function hasPermission(Employee $employee, string $action)
    {
        ...
        $incomeSpendPerms = $permissions['cash_book'];
        if (in_array('all', $incomeSpendPerms)) {
            return true;
        }

        return in_array($action, $incomeSpendPerms);
    }
    public function __construct()
    {
        //
    }
    public function viewIncomeSpend(Employee $employee)
    {
        return $this->hasPermission($employee, 'viewIncomeSpend');
    }
}
